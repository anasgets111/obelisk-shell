...
//! `power` (UPower plus power-profiles-daemon, docs/adr/0053) exist so far. `idle` used to live
//! here but moved to `hardware::idle` -- it's majority Wayland-protocol code with one D-Bus proxy
//! riding along, not a D-Bus interface in its own right (see `hardware/mod.rs`'s doc comment).
