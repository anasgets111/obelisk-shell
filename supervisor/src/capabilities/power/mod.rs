...
//!
//! `obelisk.power` combines unrelated facts: power-profiles-daemon supplies `active_profile`/`profiles`,
//! UPower supplies `on_battery`/`energy_rate`. Either half may be absent, so fields are optional
//! and never fabricated: no `"balanced"` without the profile daemon or `0` without UPower. It
