...
//! `oblisk.brightness` `nil` forever (ADR-0037's nil-until-hydrated contract).

pub mod controller;

pub use controller::{BrightnessController, BrightnessSignal, parse_set_args};

/// Every action `oblisk.brightness:invoke(...)` accepts. `dispatch` matches variants exhaustively.
