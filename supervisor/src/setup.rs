...
/// The line the generator stamps into `oblisk.lua`'s header.
///
/// Written by `stubs::render` and read here, which is a coupling across two modules with only a
/// test holding it together (`stubs::tests::the_generated_stub_stamps_a_version_that_reads_back`).
/// The alternative was compiling the whole generator into the shipped binary to keep the constant
/// next to its writer, for six lines of reader.
const VERSION_MARKER: &str = "-- GENERATED by `supervisor/src/stubs.rs` for oblisk ";

/// The version a stub directory was generated for, read from `oblisk.lua`'s header.
///
/// `None` when the directory has no readable `oblisk.lua` or its header predates the stamp. Both
/// mean the same thing to a caller: this cannot be trusted to describe the running engine.
...
pub fn run(config_dir: &Path, force: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stub_dir = match packaged_stub_dir() {
        Some(dir) => {
            // A packaged directory belongs to the package manager, so this reports a mismatch and
            // changes nothing. It means the install is half-upgraded, and rewriting files under
            // `$PREFIX/share` would be the wrong fix for that.
        ...
        None => {
            ...
            // no error anywhere, which is worse than having no completion at all.
