//! `oblisk init`: writes `.luarc.json` pointing lua-language-server at stubs, and a starter
...
/// install`, which places no data files, find them under `$PREFIX/share`.
...
/// Header marker written by `stubs::render` and read here, held by
/// `stubs::tests::the_generated_stub_stamps_a_version_that_reads_back`. Keeping it here avoids
/// shipping the generator just to share a constant.
const VERSION_MARKER: &str = "-- GENERATED by `supervisor/src/stubs.rs` for oblisk ";

/// Version from `oblisk.lua`'s header. `None` means unreadable/missing or pre-stamp, so it cannot
...
/// Embedded-stub destination without a package: `$XDG_DATA_HOME/oblisk/lua-meta`, then
...
/// `oblisk check`: evaluates the config and reports its declarations.
///
...
fn run_into(config_dir: &Path, force: bool, user_stubs: Option<&Path>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stub_dir = match packaged_stub_dir() {
        Some(dir) => {
            // Package-managed mismatch is reported, not rewritten under `$PREFIX/share`.
        ...
        None => {
            // Keep `$XDG_DATA_HOME/oblisk/lua-meta` outside config: `watcher.rs` reloads every
            // `.lua` below config, so stubs there would restyle a running bar on each refresh.
            let dir = match user_stubs {
                Some(dir) => dir.to_path_buf(),
                None => user_stub_dir()?,
            };
            // Rewrite version mismatches. These generated files are ours, not config; an old stub
            // describes removed fields and misses new ones, worse than none.
...
mod tests {
    ...
    /// An older stamped directory must be rewritten: it describes removed fields, misses new ones,
    /// and otherwise gives no error.
    #[test]
    ...
    /// An unstamped header is as untrustworthy as a wrong version.
    #[test]
