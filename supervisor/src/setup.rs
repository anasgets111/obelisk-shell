...
const VERSION_MARKER: &str = "-- GENERATED by `supervisor/src/stubs.rs` for obelisk ";

/// Version from `obelisk.lua`'s header. `None` means unreadable/missing or pre-stamp, so it cannot
/// describe the running engine.
pub fn stub_version(dir: &Path) -> Option<String> {
    let text = std::fs::read_to_string(dir.join("obelisk.lua")).ok()?;
        ...
        Some(dir) => {
            ...
            match stub_version(&dir) {
                Some(found) if found != running => println!(
                    "  stubs   {} (packaged, generated for {found}, this is obelisk {running} -- the install is inconsistent)",
                    dir.display()
                ),
...
mod tests {
    ...
    fn stubs_generated_for_another_version_are_refreshed() {
        ...
        let obelisk_lua = stubs.join("obelisk.lua");
        let aged = std::fs::read_to_string(&obelisk_lua).unwrap().replace(env!("CARGO_PKG_VERSION"), "0.0.9");
        std::fs::write(&obelisk_lua, aged).unwrap();
