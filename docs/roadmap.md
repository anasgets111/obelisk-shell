...
| CLI state cannot run an effect | `obelisk set`/`toggle` writes a `state`, and a surface reading it re-renders. That covers every declarative case and no imperative one. `IPC.qml`'s `rec toggle` starts a recording, which is a call, and the only handler a config can hang an effect on is a capability's `on_change`. Built on `obelisk.system` instead, a keybind answers up to a second late (ADR-0176) | Give a named `state` a change handler, the way a capability has one, so a CLI write can run config code at the moment it lands | Which keybinds exist, and what they do |
...
| :--- | :--- | :--- |
| Process control | `run` streams both pipes and returns an exit code, `detach` lets go of a program entirely (ADR-0188), and `session_process` outlives a reload (ADR-0175). No stdin writes, signal selection or explicit cwd/env | Which caller needs each extension? Reload ownership is settled; what is left is whether anything needs to talk to a child after starting it |
| External IPC | `obelisk set/toggle` writes named state; no callable methods or returned results | Are state writes sufficient, or do integrations need request/response commands? |
