...
| Blur and effects | No blur, shadows or arbitrary masks. A config *may* supply a fragment shader, but only as an `image.transition` over that node's two endpoints (ADR-0184): the engine ships the cross-dissolve and the ability to run one, and no effects at all. A shader over an arbitrary subtree, over content behind a surface, or as a persistent filter is still not built | Separate blurring our own images from capturing content behind a surface; settle compositor support and GPU cost. This row said config-supplied shaders were closed, and ADR-0184 overturned that half of it on the evidence that the reference's six `.frag` files are its own config's, not Quickshell's. What stays closed is the *scope*: two endpoints and a progress number is a contract that can be kept stable, and an arbitrary subtree is not |
