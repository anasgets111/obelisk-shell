...
| Capability startup | **Fixed 2026-09-07 (ADR-0156).** A swap handshake borrows the shared inbound channel and used to throw away every frame that was not the `ReadySignal` it wanted -- including the `StartCapability` frames a Candidate sends while evaluating `shell.lua`, which always arrive first. Seen live: `lock` never started, so the lock screen rendered and took keystrokes with no PAM worker behind it and the session could not be unlocked (recovered through ADR-0060's takeover marker). Those frames are now set aside and handed back to the main loop in order | Left open: nothing acknowledges a `StartCapability`, so one lost to a connection dying mid-write is still lost, and the Renderer asks exactly once per generation | Which capabilities a config asks for |
| Idle registrations outlive their generation | `reset_registrations` runs only on the in-place `Unchanged` reload path, never when a swap reaps the superseded generation, so its threshold entry stays in the notify fan-out for the life of the Supervisor. Seen live 2026-09-07 on both sides of the ADR-0156 A/B: every idle transition after a swap logs `failed to push IdleEvent ... to generation 0: no connection registered`. Harmless noise on its own, because the new generation registers its own entry -- but only since ADR-0156 made that registration arrive at all; before it, the fan-out held nothing but the dead generation and the whole dpms/lock/suspend chain stopped at the first topology reload | Reset the reaped generation's registrations in `swap_and_reap`, the same call the `Unchanged` path already makes | Which thresholds a config asks for |
...
| Lazy popups/windows | Wayland objects are created when shown | A QML-style loader just to defer surface creation. Measured 2026-09-07 against the cost that prompted the question: the 5ms cap guards one outermost signal resolve (`CpuBudget::enter` wraps a single `map`/`computed` body), not a config evaluation, and the whole 16-surface `dev-config` evaluates inside a ~25ms `oblisk check` on a **debug** build that also spawns two processes. The ~800us resolve that blew the cap did so only under a 20-way `cargo test` run, and ADR-0154's fixture move removed it. Deferring surface construction would not have touched that number in any case, because the cost is signal-chain resolution and not surface creation |
