...
**What the config VM contains.** "Lua 5.4" no longer describes it on its own. `coroutine`, `table`, `string`, `utf8`, `math`, and `package` are present in full. `debug` and `ffi` are absent, and `package.loadlib` raises, because mlua's safe mode says so. `io` is absent and `os` is cut to `time`, `date`, `clock`, and `getenv`: every other call in either library blocks the thread that dispatches Wayland events, and `process.run` is the non-blocking way to run a command (ADR-0048). A `json` global with a single `decode` function is added on top of the standard set, because a subprocess's output is otherwise a string Lua cannot read (ADR-0057). `package.path` resolves inside the config directory only, as `?.lua` and `?/init.lua`, replacing Lua's default rather than prepending to it, so a same-named system module cannot shadow the config's own (ADR-0047). A config's own modules are dropped from `package.loaded` before every re-evaluation, so editing a required file changes what the next reload sees. One Lua 5.4 detail worth knowing before splitting a config: `require` returns two values, the module and its file path, where 5.3 returned one. A call in the last position of a table constructor expands to all of its values, so `return { require(a), require(b) }` returns three surfaces, the last of which is a string. Bind each `require` to a local first.
...
The **structural** properties are the exception to all of the above and reject a `Signal` outright, because they are identities rather than values and an identity does not resolve. `node::resolve_properties` copies them through raw. They are `id`, `hover` and `scroll` on any kind, and `layer`, `anchor`, `monitor` and `namespace` on a `panel`. The `panel` four are read once per evaluation to decide whether a reload is an in-place update or a generation swap (ADR-0001), and a value that changes after that decision would move a surface between layers or monitors inside a live generation. `hover` and `scroll` name the signal a handler writes into, so a resolved one would arrive as the value instead of the handle (ADR-0062 decision 3, ADR-0069 decision 4). A `panel`'s other live fields, `keyboard_interactivity`, `exclusive`, `margin`, `width` and `height`, are deliberately not structural: layer-shell permits changing each on a mapped surface.
...
*   `entries`: `table` (Array of application structures, sorted by `name`)
    *   Application structure:
        *   `id`: `string` (The desktop file id, e.g. `"org.gnome.Nautilus"`. A subdirectory becomes a dash, per the desktop entry specification. This is `launch`'s one argument)
        *   `name`: `string` (The unlocalized `Name=`. `Name[xx]` is deliberately not read -- see ADR-0061's costs)
        *   `icon`: `string` (The `Icon=` key as written: a theme name, or an absolute path. `icon { name = ... }` takes either (ADR-0054 decision 2). `nil` for an entry with no `Icon=`)
*   `by_app_id`: `table` (Map from a toplevel's `app_id` to the same application structure, for a caller holding a window's `app_id` rather than a desktop file id -- `workspaces.active_client.class`, or a tray item's own `Id`. Keyed by exact `StartupWMClass` and exact desktop file id first, then case-folded spellings and the last dot-segment of a reverse-DNS id, with an exact key never displaced by a folded one)

> **No `Exec` field, deliberately.** The parsed command line stays in the Supervisor and is reached only through `applications:launch(id)` (ADR-0061 decision 3). A config that could read an argv could assemble a different one before handing it back to be run.
...
| `oblisk.idle:register_threshold(sec, on_idle, on_resume)` | `capability: "idle", action: "register", arguments: [sec]`<br>**Validation**: `sec` is integer, both callbacks are Lua functions. The Renderer holds the callbacks and matches an inbound `IdleEvent` by its `threshold_sec`; the callbacks take no arguments. Dropped on every re-evaluation of `shell.lua`, so registering one inside a callback that fires repeatedly is the mistake to avoid, not registering one at the top level. `oblisk.idle` is methods only and carries no signal: an idle threshold crossing is an event, not state (ADR-0032). |
...
Any property in this table or in § 5.2 accepts a `Signal` handle in place of a literal, except the structural ones § 1.2 lists and the `on_*`/`itemfn`/`key` callbacks, where a resolved `Signal` is refused for not being a function. The engine resolves the handle once per pass and then applies that property's normal rules to the result, so a `Signal` returning `"Fill"` is a valid `width` and one returning a table is the same error a literal table would be (ADR-0044). The rows below name `Signal` on only some properties; `lua-meta` spells it on every union that takes one, and that is the authority, because it is what the language server checks a config against (ADR-0081).
...
*   `hover(name)` -> `Signal` (Global. A boolean, `false` until the pointer is inside the region. Read-only to Lua: `signal:set()` refuses it, because the engine is the writer)
    *   `name`: `string` (The slot's identity, the way `state(name, initial)`'s name is. Two calls with one name are one slot, so the node declaring the region and the surface reacting to it need not be the same file, and an in-place reload keeps an open tooltip open)
