...
/// cannot be locked: acquiring anyway paints nothing, the compositor never unlocks on client
/// death (ADR-0042), and the only way out is a VT switch. Locking a user out over a config
/// omission is a denial of service, not fail-secure.
...
/// The second half of ADR-0052 decision 3's refusal. `node::lock_spec` requires only an `id`;
/// `child` is optional, so `lock { id = "x" }` legally resolves to a surface with no password
/// field, an empty input region, and a transparent buffer — exactly the black screen the decision
/// refuses, so counting tracked `lock` instances isn't enough. What matters is whether the tree
/// under the node holds a `layout::secure_submit`'s `UNLOCK_TARGET` field, the only source of the
/// `SecureSubmit` an unlock answers.
///
/// Distinct from [`NO_LOCK_DECLARED`]: one config is missing a `lock` node, the other is missing a
...
/// The other half of `finished`: the compositor refused the `lock` request immediately instead of
/// sending `locked`. Almost always another lock client already holds the session, but compositor
/// policy leaves this side of the wire unable to narrow it down further, so the message says only
/// what is known.
...
/// Exit code used when the Supervisor's control socket is gone (ADR-0059 decision 1). Nobody
/// reads it, since the process that classifies exit codes just died: it is for a journal or
/// `$status`, not a handshake. Distinct from `0` (not a clean exit) and `1` (not a failure of
...
/// whether it holds `ext_session_lock_v1` at that moment (ADR-0059 decisions 1 and 2). Pure and
/// split out because the locked half can mislead: ADR-0058 decision 4 caught the neighbouring
/// mistake, where a refusal ending "the lock screen that is on screen still stands" is true of a
/// vetoed reload but false of a process that is exiting.
...
///
/// Pure and separate: the two interesting answers are refusals, and a refusal that lives inside a
/// `&mut self` method that also talks to the compositor is untestable. See [`lock_command`].
...
/// One `SetSessionLock`, resolved against what this process already holds. `locked = true` has
/// four answers and only one takes the lock; ADR-0052 decision 3 must refuse before
/// `SessionLockState::lock` runs, since a lock granted and then found unusable is the black
/// screen it exists to prevent.
///
/// The two refusals ask different questions: `declares_lock` checks the tracked surface set,
/// `can_authenticate` checks the resolved tree — `lock { id = "x" }` resolves to nothing typable
/// (see [`LOCK_CANNOT_AUTHENTICATE`]), so `declares_lock` alone would enforce decision 3 for a
/// missing node but waive it for a declared, empty one. The compositor-cannot-lock case isn't an
/// input: `lock` answers `GlobalError::MissingGlobal` when `ext_session_lock_manager_v1` was
/// never advertised, and the caller turns that `Err` into a `Refuse` with the error's own words.
/// `locked = false` against nothing held is `Nothing`, not `Release`: `unlock_and_destroy` on a
/// lock never `locked` is the protocol's own `invalid_unlock`, which this guard prevents sending.
...
/// reporting one for a lock never granted would tell it the session went from locked to unlocked
/// when it never was. SCTK's `SessionLock::unlock` is a no-op below `is_locked()`, so nothing was
/// sent, and [`LOCK_NEVER_GRANTED`] says so instead.
...
/// whether `locked` was ever sent on this lock object.
///
/// The protocol puts both on one event: `finished` fires immediately if the compositor decides
/// `locked` will not be sent, typically because another client already holds the lock, or later,
/// after `locked` already fired, when a lock that was really up gets ended through the
/// compositor's own secure mechanism.
///
...
impl App {
    ...
    /// `ext_session_lock_surface_v1` (ADR-0052 decision 2).
    ///
    /// The entry lets the retained scene resolve this instance's tree, so an in-place reload can
    /// restyle a live lock screen, and it is the only record that this config declares a lock
    /// screen, the fact ADR-0052 decision 3 refuses a lock on the absence of.
    /// [`App::set_session_lock`] asks that question by looking for these entries.
    ///
    /// No `visible` is consulted: `layout::node::lock_spec` refuses the property outright, since
    /// the compositor owns this surface's lifetime end to end.
    ///
    /// The `wl_output` is kept rather than the output's name; [`TrackedRole::Lock`] gives the
    /// reason: `get_lock_surface` takes the proxy, and this is the one place it is already in hand.
    ...
    /// nothing if this process holds no lock.
    ///
    /// Idempotent per output, a protocol requirement: a second lock surface on one output is a
    /// `duplicate_output` error, killing the connection with the session still locked.
    /// `expand_instances` produces one `lock` instance per output per declared spec (ADR-0052
    /// decision 2); `crate::socket`'s `surface_specs` refuses configs with more than one `lock`,
    /// so "instance already has a surface" and "output already has one" always agree. The
    /// `surface: None` pattern below is that invariant's per-instance half.
    ///
    /// Three callers, one job: keep the set of lock surfaces matching outputs, whenever either set
    /// moves. Right after `lock` succeeds, since the protocol asks for immediate lock surfaces on
    /// every output, and a compositor waiting for them before sending `locked` would otherwise
    /// leave a blank frame up for its own time limit. Again on `locked`, for an output advertised
    /// inside that window. And from [`App::create_surfaces`], the hotplug path.
    ///
    /// No commit here, deliberately: every other create path ends in the initial commit its shell
    /// protocol requires, but committing before acking the first configure is a protocol error
    /// here, and the compositor sends that first configure immediately on `get_lock_surface`.
    ...
    /// the tracking entries in place.
    ///
    /// The order is [`App::destroy_surface_by_id`]'s minus its last step: `eglDestroySurface`
    /// and `wl_egl_window_destroy` first ([`App::release_bound`]), then the `SessionLockSurface`
    /// handle, whose `Drop` sends `destroy` and then destroys the `wl_surface`. That order
    /// prevents a `wl_egl_window` from pointing at an already-destroyed `wl_surface`.
    ///
    /// The entries survive because the declarations did: a `lock` instance is one retained node for
    /// as long as the config declares it, and the next lock rebuilds its surfaces through
    /// [`App::ensure_lock_surfaces`].
    ///
    /// Timing relative to the unlock is load-bearing: on the ordered-unlock path this runs after
    /// `unlock_and_destroy`, since destroying a lock surface whose output is still active while
    /// the session is locked makes the compositor fall back to a solid color, a visible flash
    /// before the desktop returns. The `finished` path has no such window: the compositor already
    /// ended the lock.
    ...
    ///
    /// `declares_lock` is counted off the tracked surface set, not the roster or the scene: that
    /// set is the one place the question has a single answer, since `create_lock` pushes an entry
    /// per `lock` instance and `destroy_surface_by_id` removes it with its output.
    ///
    /// `can_authenticate` goes to the scene: a tracked instance says a `lock` node was written,
    /// but only its resolved tree says whether anything under it could produce the
    /// `SecureSubmit` that unlocks (see [`LOCK_CANNOT_AUTHENTICATE`]); the per-tree answer is
    /// [`tree_can_authenticate`], the same predicate keyboard focus arms on. `any`, not `all`,
    /// across instances: they all resolve from the same declaration, so a single typable one
    /// means the config is correct.
    ///
        ...
        match lock_command(locked, !lock_instances.is_empty(), can_authenticate, self.session_lock.is_some()) {
            ...
            LockCommand::Acquire => match self.session_lock_state.lock(qh) {
                Ok(lock) => {
                    ...
                    // the grant would otherwise strip the password field from the tree the
                    // compositor is about to show. See `crate::socket`'s
                    // `lock_stays_authenticatable`, also why nothing but the fact of the lock is
                    // handed over: the ids the guard above answered on existed *now*, and a
                    // monitor hotplug retires and replaces them.
                ...
                // error's own words rather than a constant beside the other two: this is the one
                // refusal whose cause is outside both this shell and its config, and
    ...
    ///
    /// Reachable from exactly one place: a `SetSessionLock { locked: false }`, sent by the
    /// Supervisor only from the `pam_outcomes` arm of its `select!` loop, on a
    /// `PamOutcome::Success`. That makes "never unlock except on success" a property of one
    /// Supervisor call site, not a rule the Renderer must be trusted with. No convenience path may
    /// be added here, not on shutdown, `finished`, or a config reload: SCTK's `Drop` deliberately
    /// does not unlock either, since a Renderer that dies while locked leaves the session locked,
    /// and any self-initiated unlock here would turn a crash into an unlocked desktop.
    ///
    /// `SessionLock::unlock` is a no-op unless `is_locked()`: the in-flight case, a `lock` request
    /// whose `locked` hasn't arrived, sends nothing, and the `Drop` below sends the plain
    /// `destroy` the protocol requires instead. That is also why [`run`] round-trips before
    /// calling this: `is_locked()` flips when `locked` is dispatched, not when the compositor
    /// sends it, so without that round trip an unread `locked` would make this send a `destroy`
    /// the compositor answers with `invalid_destroy`.
    ///
    ...
    /// reported (ADR-0052 decision 4).
    ///
    /// `rescue` is the right channel: a refused lock leaves the ordinary scene on the glass, with
    /// no lock screen for the message to appear on, and `rescue` renders through the config's own
    /// surfaces. The wrong-password case is the opposite and never comes here: it happens with
    /// the lock surfaces mapped and everything else hidden, reaching the config as `oblisk.lock`
    /// instead.
    ///
    /// Nothing clears this again on purpose: a later successful evaluation clears `rescue` on its
    /// own success path (`RendererClient::handle_reevaluate`); the ordinary way out of
    /// `NO_LOCK_DECLARED` is editing the config to declare a lock screen, itself a re-evaluation.
    ...
    /// `PresentationEvidence`. Every lock state change goes through here, so the Supervisor's
    /// `lock::apply` sees each transition exactly once, and its `active` flag moves on these
    /// reports alone, never on anything it ordered itself.
...
impl SessionLockHandler for App {
    ...
    /// avoiding a blank frame. What this call catches is an output advertised inside that window,
    /// handled idempotently by [`App::ensure_lock_surfaces`].
    ///
    /// The lock handle is re-stored rather than compared against the one `lock` returned: it is
    /// the same `Arc`, SCTK's dispatch already flipped `is_locked()` on it, and storing it costs a
    /// refcount bump while removing the only way the two could ever disagree.
    ...
    ///
    /// Both set `rescue` (ADR-0052 decision 4): the test is not severity but whether a lock screen
    /// is left to read a message on. A denial never put one up and a teardown took the other away,
    /// so in both cases the ordinary scene is what the user sees and `rescue` is what it renders.
    ///
    /// Which teardown verb to send is decided by `is_locked()`; the protocol leaves no choice.
    /// `ext-session-lock-v1` requires `destroy` or `unlock_and_destroy` depending on whether
    /// `locked` was received, and its `destroy` request says: "it is a protocol error to make this
    /// request if the locked event was sent, the unlock_and_destroy request must be used instead".
    /// A post-`locked` `finished` answered with plain `destroy` is `invalid_destroy` every time,
    /// and losing the connection here is the worst outcome available: the session ends up unlocked
    /// and the shell dead, with the `rescue` message set below never reaching a surface.
    ///
    /// This is not the convenience path ADR-0042 forbids: that rule is about initiating an unlock,
    /// and the compositor initiated this one through its own secure mechanism, documented as "the
    /// compositor has decided that the session lock should be destroyed". The one path that ends a
    /// live lock is still [`App::release_session_lock`], reached only from a
    ...
    fn finished(&mut self, _conn: &Connection, _qh: &QueueHandle<Self>, session_lock: SessionLock) {
        ...
        // fall-back-to-solid-colour window here, since the compositor already decided the lock is
        // over, and these are the surfaces whose `wl_surface`s the EGL side still points at.
        self.teardown_lock_surfaces();
    ...
    /// `xdg_surface`.
    ///
    /// Everything after the lookup is [`App::bind_and_clear`], shared verbatim with the other
    /// three roles. The size is taken as given with no negotiation: a lock surface covers its
    /// output, the compositor knows that size, and committing a buffer that does not match the
    /// acked size is the protocol's own `dimensions_mismatch` error.
    ///
    ...
    /// forbids one before the first ack. `bind_and_clear` flips that to `Mapped`, binds EGL,
    /// paints the resolved tree, and `eglSwapBuffers` carries the first buffer as the commit.
    fn configure(
        &mut self,
        _conn: &Connection,
