...
--- @param player table? one entry of `obelisk.mpris`'s `players`
--- @return boolean
...
--- Video classification from one `obelisk.mpris` payload. Pure and payload-based so `lib/idle.lua`
--- can use the value from `on_change` instead of a possibly stale `computed` in its callback.
--- @param m table? `obelisk.mpris`'s payload
--- @return boolean
