...
-- cadence. `obelisk.system` pushes are load-bearing; if that timer stops, stages stop too.
...
-- `IdleInhibitor` is ignored by its own `IdleMonitor`s. Here `obelisk.idle:inhibit(reason)` takes a
...
idle.active_profile = obelisk.power:map(idle.profile_of)
...
--- watch privacy, mpris and storage, never `obelisk.idle`; [`idle.reasons`] adds foreign holders
--- back.
--- @param privacy table? `obelisk.privacy`'s payload
--- @param mpris table? `obelisk.mpris`'s payload
...
idle.reasons = computed(
    { obelisk.privacy, obelisk.mpris, store.idle, idle.manual, obelisk.idle },
...
idle.inhibited = computed({ idle.reasons, obelisk.idle }, function(reasons, foreign)
...
function idle.sync_inhibit()
    -- Our hold is excluded from `obelisk.idle.inhibitors` by `foreign_idle_inhibitors`, so readback
    ...
    if want then
        obelisk.idle:inhibit(table.concat(reasons, " + "))
    else
        obelisk.idle:release_inhibit()
    end
end
...
---
--- Recomputed every tick, not latched: when `obelisk.lock.active` goes false, lock becomes undone
...
idle.elapsed = computed({ obelisk.system, idle.since }, function(s, since)
    if since == 0 then
        return 0
    end
