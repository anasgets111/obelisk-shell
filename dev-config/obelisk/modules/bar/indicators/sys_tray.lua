...
local items = list {
    ...
    scroll = SCROLL,
    visible = has_items,
    source = computed({ obelisk.tray, obelisk.applications }, items_of),
    itemfn = function(item)
        local entry = util.app_entry(obelisk.applications:get(), item.name or item.id)
        ...
        return button {
            ...
            on_click = function(rect_, mouse_button)
                ...
                elseif mouse_button == "left" then
                    obelisk.tray:invoke("activate", item.id, 0, 0)
                elseif mouse_button == "middle" then
                    obelisk.tray:invoke("secondary_activate", item.id, 0, 0)
                end
            end,
            ...
            on_wheel = function(_, notches)
                obelisk.tray:invoke("scroll", item.id, math.floor(notches), "vertical")
            end,
            children = { face },
        }
    end,
