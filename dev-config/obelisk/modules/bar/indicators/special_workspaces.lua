...
-- set and glass while hidden; a click invokes `toggle_special`. `obelisk.workspaces.special` is
...
--
-- Use the standing window's icon when `obelisk.applications` knows its `app_id`; otherwise use the
...
local function special_button(special)
    ...
    local entry = obelisk.workspaces:map(function(w)
    ...
    local icon_name = computed({ obelisk.applications, entry }, function(applications, current)
    ...
    return button {
        ...
        on_click = function(_, mouse_button)
            ...
            obelisk.workspaces:invoke("toggle_special", name)
        end,
    }
end
...
return row {
    height = theme.item_height,
    align_v = "Center",
    visible = obelisk.workspaces:map(function(w)
    ...
    children = {
        list {
            ...
            align_v = "Center",
            source = obelisk.workspaces:map(specials_of),
