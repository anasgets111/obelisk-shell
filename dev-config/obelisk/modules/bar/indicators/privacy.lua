...
local function alert(glyph, field, on_activate)
    return icon_button(glyph, on_activate, {
        ...
        visible = util.shown_when(obelisk.privacy, users_of(field)),
    })
end
...
local mic_muted = obelisk.audio:map(function(a)
    return a ~= nil and a.source_muted == true
end)
local mic_shown = computed({ obelisk.privacy, mic_muted }, function(p, muted)
...
return row {
    ...
    visible = computed({ obelisk.privacy, mic_shown }, function(p, mic)
    ...
    children = {
        ...
        icon_button(mic_muted:map(function(muted)
            ...
            obelisk.audio:invoke("toggle_source_mute")
        end, {
