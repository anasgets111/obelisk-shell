...
--
-- Both halves read `obelisk.workspaces.active_client`; the old title used a `state` signal.
...
-- `util.app_entry` maps it to a `.desktop` entry. This is the second `obelisk.applications` consumer
-- (ADR-0061), reserved by ADR-0054 decision 5 before the capability existed.
local focused_icon = icon {
    name = computed({ obelisk.applications, obelisk.workspaces }, function(applications, workspaces)
        local client = focused(workspaces)
        if client == nil then
            return EMPTY_ICON
        end
...
return row {
    ...
    children = {
        focused_icon,
        cell(computed({ obelisk.applications, obelisk.workspaces }, function(applications, workspaces)
