...
-- Mains state is `obelisk.power` (UPower manager `OnBattery`), not the battery capability.
obelisk.power:on_change(function(p, previous)
    if previous == nil or p.on_battery == nil or p.on_battery == previous.on_battery then
        return
    end
    local b = obelisk.battery:get()
    ...
    obelisk.brightness:invoke("set", p.on_battery and 10 or 100)
end)

obelisk.battery:on_change(function(b, previous)
    ...
    -- a few seconds at every plug-in, while the asus driver still reads `Not charging`, and
    -- announcing that claimed a limit of 70 had been reached at 37%. Gating on the crossing out of
