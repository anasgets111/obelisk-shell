...
-- ## Engine pieces
--
-- `obelisk.files` follows the folder (ADR-0120), so the grid lists a `computed` without scanning.
...
---@return FileEntry[]
local filtered = computed({ obelisk.files, trimmed }, function(f, needle)
...
-- selection reads as "all" (`onMonitorOptionsChanged`), because `obelisk.screens` has no
-- `on_change`.
...
local effective_monitor = computed({ monitor, obelisk.screens }, choice_among)

local function targets_now()
    local chosen = choice_among(monitor:get(), obelisk.screens:get())
    if chosen == ALL then
...
local current_path = computed({ wallpaper.all(), obelisk.screens, effective_monitor }, function(w, screens, chosen)
    local paths = {}
    for _, screen in ipairs(screens or {}) do
...
local current_fit = computed({ wallpaper.all(), obelisk.screens, effective_monitor }, function(w, screens, chosen)
    local fits = {}
    for _, screen in ipairs(screens or {}) do
        if chosen == ALL or chosen == screen.name then
...
local folder_state = computed({ obelisk.files, filtered }, function(f, shown)
    local folder = wallpaper.folder_in(f)
...
local monitor_options = obelisk.screens:map(function(screens)
