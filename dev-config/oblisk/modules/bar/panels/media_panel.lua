...
local position_us = computed({ selected, oblisk.system, anchor, seek_base }, function(player, s, anchored, base)
    if not player then
        return -1
    end
...
local function transport(slot, icon, command, offset, size)
    return panel_action_icon(icon, function()
        ...
        if offset then
            ...
            anchor:set(os.time())
            oblisk.mpris:invoke("seek_relative", player.id, offset)
        else
            oblisk.mpris:invoke("control", player.id, command)
        end
